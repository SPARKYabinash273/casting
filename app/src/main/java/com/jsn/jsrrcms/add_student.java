...
public class add_student extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        et_user_id=(EditText)findViewById(R.id.editTextuser);
        et_name=(EditText)findViewById(R.id.editTextname);
        et_roll_no=(EditText)findViewById(R.id.editTextroll);
        et_dept=(EditText)findViewById(R.id.editTextdept);
        ...
        if(firebaseAuth.getCurrentUser()!=null){
            Toast.makeText(add_student.this, "Already signIn", Toast.LENGTH_SHORT).show();
    ...
    public void fnRegister(View view) {
        ...
        if(sum==-8){
            Toast.makeText(add_student.this, "Please Select subjects", Toast.LENGTH_SHORT).show();
            return ;
        }
        String arr[]=new String[]{ userid,name,rollno,dept,phone,email,password };
        for(int i=0;i<7;i++){
            if(TextUtils.isEmpty(arr[i])){
                Toast.makeText(add_student.this, "Block cannot be empty", Toast.LENGTH_SHORT).show();
                return ;
            }
        }
        if(password.length()<6){
            Toast.makeText(add_student.this, "Password should contain min. 6 characters", Toast.LENGTH_SHORT).show();
            return ;
        }

        Log.e("Entering ", "In fn2");
        Toast.makeText(add_student.this, "Validation Successful", Toast.LENGTH_SHORT).show();
            ...
            firebaseAuth.createUserWithEmailAndPassword(email,password).addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
                ...
                public void onComplete(Task<AuthResult> task) {
                    if (task.isSuccessful()) {
                        Toast.makeText(add_student.this, "User Registered Successfully", Toast.LENGTH_SHORT).show();
                        saveInformation(stdinfo);
                        pd.dismiss();
                    } else {
                        Toast.makeText(add_student.this, "Unable to register", Toast.LENGTH_SHORT).show();
